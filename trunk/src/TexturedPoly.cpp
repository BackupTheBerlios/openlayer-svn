...
SetTexture( const Bitmap &texture ) {
   ...
   */
   ...
   for( float y = minY; y <= maxY; y += bmpH ) {
      for( float x = minX; x <= maxX; x += bmpW ) {
         cout << "Loop begins with x: " << x << ", y: " << y << endl;
         ...
         Vec2D beginPoint, endPoint;
         
         std::vector< Vec2D > ::const_iterator start = vertices.begin();
         
         for(; start != vertices.end(); start++ ) {
            cout << "Testing start: " << start->ToString() << endl;
            if( isInside( *start, x, y, bmpW, bmpH )) {
               break;
            }
         }
         
         if( start == vertices.end() ) {
            std::vector< Vec2D > ::const_iterator beforeEnd = vertices.begin();
            
            for( int index = 0; index < vertices.size()-1; index++ ) {
               beforeEnd++;
            }
            
            std::vector< Vec2D > ::const_iterator iterPrev = beforeEnd;
            bool beginFound = false;
            
            for( std::vector< Vec2D > ::const_iterator iter = vertices.begin(); iter != vertices.end();
                 iter++ ) {
               for( int i = 0; i < 4; i++ ) {
                     Line line( iterPrev, iter );
                     
                     if( textureEdges[i]->Collides( line )) {
                        if( !beginFound ) {
                           beginFound = true;
         ...
         else if( start == vertices.begin()) {
            std::vector< Vec2D > ::const_reverse_iterator newStart = vertices.rbegin();
            int index = vertices.size()-1;
            
            for(; newStart != vertices.rend(); newStart++ ) {
               if( !isInside( *newStart, x, y, bmpW, bmpH )) {
                  break;
               }
               index--;
            }
            cout << "StartOverBegin: " << index << endl;
            
            if( newStart != vertices.rend() ) {
               start = vertices.begin();
               for( int i = 0; i < index; i++ ) {
                  start++;
               }
            }
            cout << "Start now: " << start->ToString() << endl;
         }
         
         std::vector< Vec2D > ::const_iterator end = start;
         end++;
         std::vector< Vec2D > ::const_iterator beforeEnd = end;
         //bool overTheEnd = false;
         
         for(; end != vertices.end(); end++ ) {
            if( !isInside( *end, x, y, bmpW, bmpH )) {
               break;
            }
         ...
         if( end == vertices.end() ) {
            cout << "OverTheEnd" << endl;
            //overTheEnd = true;
            
            for( end = vertices.begin(); end != vertices.end(); end++ ) {
               cout << "Testing end: " << end->ToString() << endl;
               if( !isInside( *end, x, y, bmpW, bmpH )) {
                  break;
               }
            ...
            cout << "BeforeEnd: " << beforeEnd->ToString() << endl;
            if( end == vertices.end() ) {
               cout << "All inside" << endl;
               
               end = beforeEnd;
               
               beforeEnd = vertices.begin();
               for( std::vector< Vec2D > ::const_iterator iter = vertices.begin(); iter != end; iter++ ) {
                  beforeEnd++;
               }
               
               // All vertices are inside the texture region
               start = vertices.begin();
               //overTheEnd = false;
               cout << "All inside end" << endl;
            }
            
            // Find the start and the end of the slice parts
            
            std::vector< Vec2D > ::const_iterator startNext = start;
            startNext++;
            
            if( startNext == vertices.end() ) {
               startNext = vertices.begin();
            }
            
            Line beginSegment( *start, *startNext );
            
            for( int i = 0; i < 4; i++ ) {
               if( textureEdges[i]->Collides( beginSegment )) {
                  //cout << textureEdges[i]->ToString() << endl;
                  beginPoint = textureEdges[i]->GetIntersectionPoint( beginSegment );
                  break;
               }
            }
            
            Line endSegment( *beforeEnd, *end );
            
            for( int i = 0; i < 4; i++ ) {
               if( textureEdges[i]->Collides( endSegment )) {
                  endPoint = textureEdges[i]->GetIntersectionPoint( endSegment );
                  break;
               }
            }
         }
         ...
         if( isOK ) {
            ...
            Circle( beginPoint, 10 ).Draw( Rgba::YELLOW );
            
            slice->AddPart( new SlicePart( beginPoint.y, y + bmpH, beginPoint.x, x, y, texture ));
            ...
            while( iter != end ) {
               ...
               slice->AddPart( new SlicePart( iter->y, y + bmpH, iter->x, x, y, texture ));
            ...
            slice->AddPart( new SlicePart( endPoint.y, y + bmpH, endPoint.x, x, y, texture ));
            
            Circle( endPoint, 5.0 ).Draw( Rgba::GREEN );
